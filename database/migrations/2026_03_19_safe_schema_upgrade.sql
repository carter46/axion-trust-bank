/* 
  ...
  Includes: core schema, banking UX (KYC fields), dynamic KYC config, admin audit logs.

  How to run:
  - phpMyAdmin/Adminer: select DB, paste + run this whole file
  - CLI: mysql -u USER -p DB_NAME < 2026_03_19_safe_schema_upgrade.sql

  Do NOT run seed scripts (e.g. andy_transactions) on production unless you intend to import demo data.
*/
...
SET @sql := IF(@has_schema_migrations > 0,
  'INSERT INTO schema_migrations (version, migration_name, migration_file, status)
   SELECT ''2026.06.05'', ''safe_schema_upgrade'', ''2026_03_19_safe_schema_upgrade.sql'', ''success''
   WHERE NOT EXISTS (
     SELECT 1 FROM schema_migrations WHERE version = ''2026.06.05'' AND migration_name = ''safe_schema_upgrade''
   );',
  'SELECT 1;'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
